...
function preloadHandlebarsTemplates() {
  ...
  const TEMPLATE_PATHS = ['systems/yzesrd-vtt/templates/components/roll/roller.hbs', 'systems/yzesrd-vtt/templates/components/roll/roll-push-select-dialog.hbs', 'systems/yzesrd-vtt/templates/components/roll/roll-chatcard-tooltip-partial.hbs', 'systems/yzesrd-vtt/templates/components/roll/roll-chatcard.hbs', 'systems/yzesrd-vtt/templates/components/roll/roll-chatcard-infos-partial.hbs', 'systems/yzesrd-vtt/templates/components/roll/roll-askdie-dialog.hbs', 'systems/yzesrd-vtt/templates/components/dialog/table-draw-dialog.hbs', 'systems/yzesrd-vtt/templates/components/dialog/range-picker-dialog.hbs', 'systems/yzesrd-vtt/templates/actor/inventory-partial.hbs', 'systems/yzesrd-vtt/templates/actor/vehicle/vehicle-sheet.hbs', 'systems/yzesrd-vtt/templates/actor/vehicle/vehicle-seat-partial.hbs', 'systems/yzesrd-vtt/templates/actor/loot/loot-sheet.hbs', 'systems/yzesrd-vtt/templates/actor/character/character-limited-sheet.hbs', 'systems/yzesrd-vtt/templates/actor/character/character-sheet.hbs', 'systems/yzesrd-vtt/templates/actor/character/sheet-tabs/stats-tab.hbs', 'systems/yzesrd-vtt/templates/actor/character/sheet-tabs/bio-tab.hbs', 'systems/yzesrd-vtt/templates/actor/character/sheet-tabs/mods-tab.hbs', 'systems/yzesrd-vtt/templates/actor/character/sheet-tabs/combat-tab.hbs', 'systems/yzesrd-vtt/templates/actor/character/sheet-tabs/rebellion-bio-tab.hbs', 'systems/yzesrd-vtt/templates/actor/character/sheet-tabs/inventory-tab.hbs', 'systems/yzesrd-vtt/templates/item/actions-partial.hbs', 'systems/yzesrd-vtt/templates/item/attacks-partial.hbs', 'systems/yzesrd-vtt/templates/item/item-sheet.hbs', 'systems/yzesrd-vtt/templates/item/active-effects-partial.hbs', 'systems/yzesrd-vtt/templates/item/item-chatcard.hbs', 'systems/yzesrd-vtt/templates/item/modifiers-partial.hbs'];
  const paths = TEMPLATE_PATHS;
  console.log('Year Zero Engine SRD RPG | Loading Handlebars templates:', paths);
  return foundry.applications.handlebars.loadTemplates(paths);
}

export function registerCustomItemTypesTemplates() {
  const customItemTypesTemplates = Object.values(YZSRD.customItemTypesTemplates);
...
function registerHandlebarsHelpers() {
  ...
  Handlebars.registerHelper('scoreSelector', function (target, options) {
    ...
    for (const [score, size] of YZSRD.scoreMap) {
  ...
  Handlebars.registerHelper('boxes', function (field, options) {
    ...
    for (let i = 0; i < max; i++) {
      if (i === 10) str += '<br/>';
      if (value > i) str += `${YZSRD.Icons.boxes.full}`;
      else str += `${YZSRD.Icons.boxes.empty}`;
    }
    if (loss < 0) {
      str += '<span class="loss">';
      for (; loss < 0; loss++) {
        str += `${YZSRD.Icons.boxes.lost}`;
      }
      str += '</span>';
    }
    str += '</a>';
  ...
  Handlebars.registerHelper('createNewItemButton', function (type) {
    const title = game.i18n.format('YZSRD.CreateNewItem', {
      type: game.i18n.localize(`TYPES.Item.${type}`),
    });
    const str = `<a class="btn item-create" data-type="${type}" data-tooltip="${title}">${YZSRD.Icons.buttons.plus}</a>`;
    return new Handlebars.SafeString(str);
  });
}
