...
>${YZSRD.Icons.links.rolltable}${title}</a>`;
...
>${YZSRD.Icons.links.choice}${result}</a>`;
 ...
 */
const ACTOR_PATTERN = /@YzeSRDActor\[(.+?)\](?:{(.+?)})?/gm;
...
async function actorEnricher(match, _options) {
  ...
  // Builds attributes.
  const attributes = [];
  for (const attr of YZSRD.attributes) {
    const value = sys.attributes[attr].value;
    const text = game.i18n.localize(`YZSRD.ATTRIBUTE.${attr.toUpperCase()}`)
      + ` ${YZSRD.dieMap.get(+value)}`;
    attributes.push(text);
  }

  // Builds skills.
  const skills = [];
  for (const sk of YZSRD.skills) {
    const value = sys.skills[sk].value;
    if (value <= 6) continue;
    const text = game.i18n.localize(`YZSRD.SKILL.${sk.capitalize()}`)
      + ` ${YZSRD.dieMap.get(+value)}`;
    skills.push(text);
  }
 ...
 * - $2: Given name overriding the default one
 */
const WEAPON_PATTERN = /@YzeSRDWeapon\[(.+?)\](?:{(.+?)})?/gm;
...
<div class="flbr-table green" style="height: initial; max-width: 40%;">
  <table>
    <tbody>
      ...
      <tr><td>TYPE: <b>${game.i18n.localize(YZSRD.damageTypes[atk.damageType])}</b></td></tr>
      <tr><td>MIN. RANGE: <b>${game.i18n.localize(YZSRD.ranges[atk.range?.min])}</b></td></tr>
      <tr><td>MAX. RANGE: <b>${game.i18n.localize(YZSRD.ranges[atk.range?.max])}</b></td></tr>
      <tr><td>AVAILABILITY: <b>${game.i18n.localize(YZSRD.availabilities[sys.availability])}</b></td></tr>
 ...
 * - $2: Given name overriding the default one
 */
const HANDOUT_PATTERN = /@YzeSRDHandout\[(.+?)\](?:{(.+?)})?/gm;
...
/* ------------------------------------------ */
/*  BLADE RUNNER SYMBOL                       */
/*   Generates a Year Zero Engine SRD symbol          */
/* ------------------------------------------ */
...
async function bladeRunnerSymbolEnricher(match) {
  ...
  symbolDoc.className = 'yzesrd-vtt-symbols';
