...
export function addChatMessageContextOptions(_html, options) {
  ...
  options.push({
    name: game.i18n.localize('YZSRD.CHAT_ACTION.ApplyDamage'),
    icon: YZSRD.Icons.buttons.attack,
  ...
  for (const defenderToken of defenderTokens) {
    if (defenderToken.actor.type === ACTOR_TYPES.LOOT) {
      ui.notifications.info(
        game.i18n.format('YZSRD.COMBAT.SkippedDefender', {
          name: `<b>${defenderToken.name}</b>`,
        }) + ' <i>(' + game.i18n.localize('YZSRD.COMBAT.CannotBeDamaged') + ')</i>',
      );
      continue;
    }
    if (!s) break;
    ...
    if(defenderTokens.size > 1 && !boom) {
      n = await YzeSRDDialog.rangePicker({
        title: game.i18n.localize('YZSRD.DIALOG.AssignSuccesses'),
        description: game.i18n.format('YZSRD.DIALOG.AssignSuccessesHint', {
          name: `<b>${defenderToken.name}</b>`,
        }),
        value: s,
      });
...
export function addChatListeners(html) {
  html.addEventListener('click', ev => {
    if (ev.target.closest('.yzesrd-vtt-display-manual')) game.bladerunner.macros.displayManual.call(ev.target, ev);
  });
  html.addEventListener('click', ev => {
...
async function _onCritApply(event) {
  ...
  if (!item || !actor) {
    ui.notifications.error(game.i18n.localize('YZSRD.COMBAT.CritApplyErrorMessage'));
    return;
  }
  const itemDoc = await foundry.utils.fromUuid(item);
  const actorDoc = await foundry.utils.fromUuid(actor);
  if (!itemDoc || !actorDoc) {
    ui.notifications.error(game.i18n.localize('YZSRD.COMBAT.CritApplyErrorNotFound'));
    return;
  }
