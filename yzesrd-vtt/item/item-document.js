...
export default class YzeSRDItem extends Item {
  ...
  get isPhysical() {
    return YZSRD.physicalItems.includes(this.type);
  }
  ...
  async roll() {
    ...
    if (this.system.consumable) {
      ...
      ui.notifications.info(
        game.i18n.format('YZSRD.ItemConsumeNotif', {
          name: this.name,
          qty: consumed,
        }),
      );
    ...
    if (this.isOffensive) {
      ...
      if (this.attacks.length > 1) {
        attackId = await YzeSRDDialog.choose(
          this.attacks.map(a => {
            const atk = new ItemAttack(this.system.attacks[a.id]);
            return [a.id, atk.title];
          }),
          `${this.detailedName}: ${game.i18n.localize('YZSRD.DIALOG.ChooseAttack')}`,
        );
      }
      else {
  ...
  async _rollSpecial(value) {
    ...
    const execute = autoArmorRoll || await Dialog.confirm({
      title,
      content: `<p>${game.i18n.format('YZSRD.ROLLER.RollSpecialHint', {
        name: `<b>${this.name}</b>`,
      })}</p>`,
    });
    if (!execute) return;
