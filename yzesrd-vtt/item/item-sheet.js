 ...
 */
export default class YzeSRDItemSheet extends foundry.appv1.sheets.ItemSheet {
  ...
  _getHeaderButtons() {
    ...
    if (this.item.hasAction && this.item.actor) {
      myButtons.push({
        label: game.i18n.localize('YZSRD.SHEET_HEADER.ItemRoll'),
        class: 'item-roll',
        icon: 'fas fa-dice',
    ...
    if (this.item.type === ITEM_TYPES.ARMOR) {
      myButtons.push({
        label: game.i18n.localize('YZSRD.SHEET_HEADER.ArmorRoll'),
        class: 'item-roll',
      ...
      myButtons.push({
        label: game.i18n.localize('YZSRD.SHEET_HEADER.BlastPowerRoll'),
        class: 'blast-roll',
        icon: 'fas fa-bomb',
    ...
    myButtons.push({
      label: game.i18n.localize('YZSRD.SHEET_HEADER.ItemPost'),
      class: 'item-post',
  ...
  _onAddItemAction(event) {
    ...
    const itemAction = new ItemAction(null, {
      name: game.i18n.localize('YZSRD.ItemNewAction'),
      // item: this,
    });
  ...
  _onAddItemAttack(event) {
    ...
    const itemAttack = new ItemAttack({
      name: game.i18n.localize('YZSRD.ItemNewAttack'),
      // item: this,
    });
  ...
  _onAddActiveEffect(event) {
    ...
    return this.item.createEmbeddedDocuments('ActiveEffect', [{
      name: game.i18n.localize('YZSRD.ACTIVE_EFFECT.New'),
  ...
  _onBlastChange(event) {
    ...
    if (!(blast in YZSRD.blastPowerMap)) return;
    if (this.item.attacks.length <= 0) return;
    const { damage, crit } = YZSRD.blastPowerMap[blast];
