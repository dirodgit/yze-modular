...
export default class BRRollHandler extends FormApplication {
  ...
  constructor({
    ...
    /** @typedef {import('@actor/actor-document').default} YzeSRDActor */
    ...
    /** @typedef {import('@item/item-document').default} YzeSRDItem */
  ...
  getRollOptions() {
    ...
    return {
      ...
      damage: this.damage,
      damageType: this.damageType,
      damageTypeName: YZSRD.damageTypes[this.damageType],
      crit: this.crit,
  ...
  static async applyDamage({ attributeTrauma, options: { attributeKey } }, speaker) {
    ...
    const cap = YZSRD.pushTraumaMap[nature]?.[attributeKey];
    if (!cap) {
      console.error(`YZSRD | ApplyDamageNoCapacityKey → nature: ${nature}, attributeKey: ${attributeKey}`);
    ...
    if (value === 0) {
      ui.notifications.info('YZSRD.YouAreBroken', { localize: true });
    }
  ...
  static async applyCrit(roll) {
    ...
    if (!actor && game.user.isGM) {
      ...
      actor = await chooseActor(actors, {
        title: game.i18n.localize('YZSRD.CRIT.CriticalInjury'),
        notes: game.i18n.localize(game.i18n.format('YZSRD.CRIT.ChooseActor')),
      });
    }
    if (!actor) {
      return ui.notifications.error(game.i18n.format('YZSRD.CRIT.NoActorSelected'));
    }

    if (actor.id == game.user.character?.id) {
      return ui.notifications.warn(game.i18n.format('YZSRD.CRIT.DrawCritOwnCharacter'));
    }
  ...
  static async askDie(lowest = 6) {
    ...
    return Dialog.prompt({
      title: game.i18n.localize('YZSRD.ROLLER.AddDie'),
      content,
      label: game.i18n.localize('YZSRD.OK'),
      callback: html => Number(html[0].querySelector('form').diesize.value),
      rejectClose: false,
      options: {
        width: 100,
        classes: ['yzesrd-vtt', 'dialog'],
        minimizable: false,
      },
    });
  }
  ...
  static async selectPush(dice) {
    return Dialog.wait({
      title: game.i18n.localize('YZSRD.ROLLER.SelectDiceToPush'),
      ...
      buttons: {
        ...
        cancel: {
          label: game.i18n.localize('YZSRD.Cancel'),
          icon: '<i class="fas fa-times"></i>',
          callback: () => false,
        },
      },
    }, {}, {
      classes: ['yzesrd-vtt', 'dialog', 'dice-push-select'],
    });
  }
}
