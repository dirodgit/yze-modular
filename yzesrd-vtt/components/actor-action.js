...
export class ActorAction {
  ...
  async execute(actor) {
    ...
    const attribute = this.attribute || YZSRD.skillMap[this.skill];
    const title = `${actor.name}: ${this.label} (${game.i18n.localize(`YZSRD.SKILL.${this.skill.capitalize()}`)})`;
