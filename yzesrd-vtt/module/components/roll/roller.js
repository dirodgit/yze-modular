...
import { ACTOR_TYPES, ITEM_TYPES, SYSTEM_ID } from '../../system/constants.js';
import { chooseActor, getActiveActor } from '../../utils/get-actor.js';
 ...
 * Supports Year Zero Engine SRD standard rolls.
 * @see Dialog
...
export default class BRRollHandler extends FormApplication {
   ...
   */
  constructor({
    title = 'Year Zero Engine SRD RPG',
    actor = {},
    items = [],
    attributeKey = null,
    skillKey = null,
  ...
  /**
   * Creates a Year Zero Engine SRD Roll Handler FormApplication.
  ...
  getRollOptions() {
    ...
    return {
      ...
      damage: this.damage,
      damageType: this.damageType,
      damageTypeName: YZE.damageTypes[this.damageType],
      crit: this.crit,
  ...
  static async applyDamage({ attributeTrauma, options: { attributeKey } }, speaker) {
    ...
    const cap = YZE.pushTraumaMap[nature]?.[attributeKey];
    if (!cap) {
      console.error(`YZE | ApplyDamageNoCapacityKey → nature: ${nature}, attributeKey: ${attributeKey}`);
    ...
    if (value === 0) {
      ui.notifications.info('YZE.YouAreBroken', { localize: true });
    }
  ...
  static async applyCrit(roll) {
    ...
    if (!actor && game.user.isGM) {
      ...
      actor = await chooseActor(actors, {
        title: game.i18n.localize('YZE.CRIT.CriticalInjury'),
        notes: game.i18n.localize(game.i18n.format('YZE.CRIT.ChooseActor')),
      });
    }
    if (!actor) {
      return ui.notifications.error(game.i18n.format('YZE.CRIT.NoActorSelected'));
    }

    if (actor.id == game.user.character?.id) {
      return ui.notifications.warn(game.i18n.format('YZE.CRIT.DrawCritOwnCharacter'));
    }
  ...
  static async askDie(lowest = 6) {
    const template = 'systems/yzesrd-vtt/templates/components/roll/roll-askdie-dialog.hbs';
    const content = await foundry.applications.handlebars.renderTemplate(template, { lowest });
    return Dialog.prompt({
      title: game.i18n.localize('YZE.ROLLER.AddDie'),
      content,
      label: game.i18n.localize('YZE.OK'),
      callback: html => Number(html[0].querySelector('form').diesize.value),
      rejectClose: false,
      options: {
        width: 100,
        classes: ['yze-modular', 'dialog'],
        minimizable: false,
      },
    });
  }
  ...
  static async selectPush(dice) {
    return Dialog.wait({
      title: game.i18n.localize('YZE.ROLLER.SelectDiceToPush'),
      content: await foundry.applications.handlebars.renderTemplate(
        'systems/yzesrd-vtt/templates/components/roll/roll-push-select-dialog.hbs',
        { dice },
      ),
      buttons: {
        ok: {
        ...
        cancel: {
          label: game.i18n.localize('YZE.Cancel'),
          icon: '<i class="fas fa-times"></i>',
          callback: () => false,
        },
      },
    }, {}, {
      classes: ['yze-modular', 'dialog', 'dice-push-select'],
    });
  }
}
