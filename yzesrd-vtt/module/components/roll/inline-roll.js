import { ATTRIBUTES, SKILLS } from '../../system/constants.js';
...
export function overrideInlineRollListener() {
  ...
  document.body.addEventListener('click', async event => {
    ...
    if (event.target?.closest('a.inline-roll')) {
      ...
      if (/np|p(?:\d+|@maxPush)/i.test(formula)) {
        ...
        const roll = game.yzmodular.roll.create(formula, actor?.getRollData());
        await roll.roll();
  ...
  console.log('Year Zero Engine SRD RPG | Inline-roll listener overridden.');
}
...
function _createTitle(formula, flavor, actor, attributeKey, skillKey) {
  let attr, skill;
  if (attributeKey) attr = game.i18n.localize(`YZE.ATTRIBUTE.${attributeKey.toUpperCase()}`);
  if (skillKey) skill = game.i18n.localize(`YZE.SKILL.${skillKey.capitalize()}`);
