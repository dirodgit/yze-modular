import { YZE } from '../system/config.js';
import {
...
import Modifier from '../components/item-modifier.js';
import BRRollHandler from '../components/roll/roller.js';
import BladeRunnerDialog from '../components/dialog/dialog.js';
import CrewCollection from '../components/vehicle-crew.js';
...
/**
 * Year Zero Engine SRD Actor Document.
 * @extends {Actor}
 */
export default class BladeRunnerActor extends Actor {
    ...
    switch (this.type) {
      case ACTOR_TYPES.CHAR:
        return [ACTOR_SUBTYPES.PC, ACTOR_SUBTYPES.REPLICANT_REBELLION].includes(this.system.subtype)
          ? YZE.maxPushMap[this.system.nature]
          : 0;
      default:
        return 1;
    }
  ...
  _prepareCapacities() {
    ...
    for (const cap of Object.values(CAPACITIES)) {
      /** @type {ActorCapacity} */
      const capacity = this.system[cap];
      const capData = YZE.capacitiesMap[cap];
      // Proceeds if it exists in the character.
      if (capacity && capData) {
        // Gets the nature modifier.
        const natureModifier = YZE.natureModifierMap[this.system.nature]?.[cap] ?? 0;
    ...
    switch (this.type) {
      case ACTOR_TYPES.CHAR:
        ...
        if (!this.system.attributes || !this.system.skills) {
          throw new TypeError(`YZE | "${this.type}" has No attribute nor skill`);
        }
        if (foundry.utils.isEmpty(this.system.skills)) {
          ...
          for (const attribute in this.system.attributes) {
            updateData[`system.attributes.${attribute}.value`] = YZE.startingAttributeLevel;
          }
          // Builds the list of skills and sets their default values.
          for (const skill in YZE.skillMap) {
            updateData[`system.skills.${skill}.value`] = YZE.startingSkillLevel;
          }
        }
        break;
      case ACTOR_TYPES.VEHICLE:
  ...
  async addVehicleOccupant(actor) {
    ...
    if (this.crew.has(actor.id)) {
      return ui.notifications.info('YZE.VEHICLE.NotifPassengerAlreadyPresent', { localize: true });
    }
  ...
  async updateCharacterManeuverability(value) {
    ...
    const mvr = YZE.vehicleAttribute;
  ...
  async roll(rollData = {}, options = {}) {
    ...
    if (!rollData.title) rollData.title = `${this.name}: ${game.i18n.localize('YZE.SHEET_HEADER.GenericRoll')}`;
  ...
  async rollStat(attributeKey, skillKey, options = {}) {
    ...
    const attributeName = game.i18n.localize(`YZE.ATTRIBUTE.${attributeKey.toUpperCase()}`);
    const skillName = skillKey ? game.i18n.localize(`YZE.SKILL.${skillKey.capitalize()}`) : null;
   ...
   */
  async rollResolve() {
    const title = game.i18n.localize('YZE.ROLLER.ResolveTest');
    const execute = await Dialog.confirm({
      title,
      content: `<p>${game.i18n.localize('YZE.ROLLER.ResolveTestHint')}</p>`,
    });
    if (!execute) return;
    ...
    if (roll.baneCount) {
      ui.notifications.info(game.i18n.format('YZE.ROLLER.ResolveTestFailed', {
        name: this.name,
      }), {
        permanent: true,
      });
  ...
  async rollRamming() {
    ...
    const driving = YZE.vehicleSkill;
    const hullDamage = Math.ceil(this.system.hull.max / 2);

    return this.roll({
      title: `${this.name} | ${actor.name}: `
        + `${game.i18n.localize('YZE.VEHICLE.Action.Ramming')} `
        + `(${game.i18n.localize(`YZE.SKILL.${driving.capitalize()}`)})`,
      actor: actor,
      attributeKey: YZE.vehicleAttribute,
      skillKey: driving,
  ...
  async _applyDamage(damage, capacity, ignoreArmor = false) {
    ...
    if (!(capacity in this.system)) {
      throw new Error(`YZE | BladeRunnerActor.applyDamage → Non-existent capacity "${capacity}"`);
    }
  ...
  async getArmorAblation() {
    ...
    // For vehicles:
      ...
      const armorRoll = game.yzmodular.roll.create(`2d${this.system.armor}p0`, {}, {
        name: `${this.name}: ${game.i18n.localize('YZE.ItemArmor')}`,
        yzur: true,
      });
  ...
  async crashVehicle(massive = false) {
    ...
    let formula = massive ? YZE.vehicleMassiveCrashDamage : YZE.vehicleCrashDamage;

    const toCrash = await Dialog.confirm({
      title: `${this.name}: ${game.i18n.localize('YZE.VEHICLE.Action.Crash')}`,
      content: game.i18n.format('YZE.VEHICLE.Action.CrashDialog', {
    ...
    const crashDamageRoll = foundry.dice.Roll.create(formula, rollData, {
      name: `${this.name}: ${game.i18n.localize('YZE.VEHICLE.Action.Crash')}`,
    });
    await crashDamageRoll.roll();
    const crashDamageRollMessage = await crashDamageRoll.toMessage({
      flavor: `${game.i18n.localize('YZE.VEHICLE.Action.Crash')}: ${formula}`,
    });
  ...
  async explodeVehicle() {
    ...
    const toExplode = await Dialog.confirm({
      title: `${this.name}: ${game.i18n.localize('YZE.VEHICLE.Action.Explode')}`,
      content: game.i18n.localize('YZE.VEHICLE.Action.ExplodeHint'),
    });
    if (!toExplode) return;

    const blastPower = YZE.vehicleExplosionBlastPower;
    const blast = YZE.blastPowerMap[YZE.vehicleExplosionBlastPower];

    /** @type {import('yzur').YearZeroRoll} */
    const damageType = DAMAGE_TYPES.PIERCING;
    const blastRoll = game.yzmodular.roll.create(`2d${blastPower}p0`, {}, {
      damage: blast.damage,
      damageType,
      damageTypeName: game.i18n.localize(YZE.damageTypes[damageType]),
      crit: blast.crit,
      yzur: true,
    });
    await blastRoll.roll();
    const blastRollMessage = await blastRoll.toMessage({
      flavor: game.i18n.localize('YZE.VEHICLE.Action.Explode'),
    });
  ...
  async _drawCritForCharacter(damageType, severity, critRollFormula) {
    ...
    const { results } = await BladeRunnerDialog.drawTable(tables, {
      title: `${this.name}: ${game.i18n.localize('YZE.CRIT.DrawCrit')}`,
    ...
    const itemUuid = await BladeRunnerDialog.choose(
      ...
      `${this.name}: ${game.i18n.localize('YZE.CRIT.ChooseCrit')}`,
    ...
    const resultText = await foundry.applications.ux.TextEditor.enrichHTML(
      game.i18n.format('YZE.CHAT.CritInflicted', {
        name: `<b>${this.name}</b>`,
        crit: `@UUID[${crit.uuid}]`,
      }));
    const content = await foundry.applications.handlebars.renderTemplate(template, {
  ...
  async _drawCritForVehicle(severity, critRollFormula) {
    ...
    const { results } = await BladeRunnerDialog.drawTable(tables, {
      title: `${this.name}: ${game.i18n.localize('YZE.CRIT.DrawVehicleDamage')}`,
    ...
    const resultIndex = await BladeRunnerDialog.choose(
      ...
      `${this.name}: ${game.i18n.localize('YZE.CRIT.ChooseVehicleDamage')}`,
