import BladeRunnerActorSheet from '../actor-sheet.js';
import { SYSTEM_ID, ACTOR_TYPES, ACTOR_SUBTYPES, SKILLS, ATTRIBUTES } from '../../system/constants.js';
import { YZE } from '../../system/config.js';

/**
 * Year Zero Engine SRD RPG Actor Sheet for Character.
...
export default class BladeRunnerVehicleSheet extends BladeRunnerActorSheet {
  ...
  async getData(options) {
    ...
    sheetData.actions = game.yzmodular.actions.filter(a => a.actorType === this.actor.type);
    return sheetData;
  }
  ...
  async _onDropActor(event, data) {
    ...
    return ui.notifications.info(`YZE.VEHICLE.NotifDropCrew${actor.type.capitalize()}`, { localize: true });
  }
  ...
  async dropCrew(actor) {
    if (this.vehicle.crew.full) {
      return ui.notifications.info('YZE.VEHICLE.NotifCrewFull', { localize: true });
    }
    if (
      ...
      const toCopy = await Dialog.confirm({
        title: game.i18n.localize('YZE.VEHICLE.CreateActor'),
        content: game.i18n.format('YZE.VEHICLE.CreateActorHint', {
          name: actor.name,
        }),
      });
      if (toCopy) {
  ...
  activateListeners(html) {
    ...
    new foundry.applications.ux.ContextMenu.implementation(html[0], '.hull', [
      {
        name: game.i18n.localize('YZE.VEHICLE.IncreaseHull'),
        icon: YZE.Icons.buttons.plus,
        callback: () => this.vehicle.update({
          'system.hull.max': this.vehicle.system.hull.max + 1,
        }),
        condition: () => this.vehicle.system.hull.max < YZE.maxVehicleHull,
      },
      {
        name: game.i18n.localize('YZE.VEHICLE.DecreaseHull'),
        icon: YZE.Icons.buttons.minus,
        callback: () => this.vehicle.update({
          'system.hull.max': this.vehicle.system.hull.max - 1,
        }),
    ...
    new foundry.applications.ux.ContextMenu.implementation(html[0], '.vehicle-seat:not(.button)', [
      {
        name: game.i18n.localize('YZE.VEHICLE.EditPassenger'),
        icon: YZE.Icons.buttons.edit,
        callback: el => game.actors.get(el.data('occupant-id'))?.sheet.render(true),
        condition: el => el.hasClass('occupied'),
      },
      {
        name: game.i18n.localize('YZE.VEHICLE.RemovePassenger'),
        icon: YZE.Icons.buttons.delete,
        callback: el => this.vehicle.removeVehicleOccupant(el.data('occupant-id')),
        condition: el => el.hasClass('occupied'),
      },
      {
        name: game.i18n.localize('YZE.VEHICLE.DeleteSeat'),
        icon: YZE.Icons.buttons.remove,
        callback: () => this.vehicle.update({
  ...
  async _onActionRoll(event) {
    ...
    const action = game.yzmodular.actions.get(actionKey);
    if (!action) return;
  ...
  _onManeuverabilityRoll(event) {
    ...
    return this.vehicle.roll({
      title: `${this.vehicle.name}: ${game.i18n.localize('YZE.ATTRIBUTE.MVR')}`,
  ...
  _onVehicleArmorRoll(event) {
    event.preventDefault();
    return this.vehicle.roll({
      title: `${this.vehicle.name}: ${game.i18n.localize('YZE.ItemArmor')}`,
