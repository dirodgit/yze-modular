...
import { ACTOR_TYPES, SETTINGS_KEYS, SYSTEM_ID } from '../system/constants.js';
import { enrichTextFields } from '../utils/string-util.js';
import ActorSheetConfig from './actor-sheet-config.js';

/**
 * Year Zero Engine SRD RPG Actor Sheet.
...
export default class BladeRunnerActorSheet extends foundry.appv1.sheets.ActorSheet {
  ...
  async _onDropItemCreate(itemData) {
    ...
    const alwaysAllowedItems = YZE.physicalItems;
    const allowedItems = YZE.allowedItems;
    let allowed = true;
    ...
    if (!allowed) {
      const msg = game.i18n.format('YZE.ActorSheet.NotifWrongItemType', {
        type: game.i18n.localize(`TYPES.Item.${type}`),
        actor: game.i18n.localize(`TYPES.Actor.${this.actor.type}`),
      });
      console.warn(`YZE | ${msg}`);
  ...
  _getHeaderButtons() {
    ...
    const myButtons = [
      {
        label: game.i18n.localize('YZE.SHEET_HEADER.GenericRoll'),
        class: 'custom-roll',
        icon: 'fas fa-dice',
  ...
  _onActionRoll(event) {
    ...
    return game.yzmodular.actions.get(actionKey)?.execute(this.actor);
  }
  ...
  _onItemCreate(event) {
    ...
    const itemName = game.i18n.format('YZE.NewItem', {
      type: game.i18n.localize(`TYPES.Item.${type}`),
    });
  ...
  _onAddActiveEffect(event) {
    ...
    return this.actor.createEmbeddedDocuments('ActiveEffect', [{
      label: game.i18n.localize('YZE.ACTIVE_EFFECT.New'),
