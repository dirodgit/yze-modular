...
import { ITEM_TYPES, RANGES, SETTINGS_KEYS, SKILLS, SYSTEM_ID } from '../system/constants.js';
...
import ItemAction from '../components/item-action.js';
import ItemAttack from '../components/item-attack.js';
...
export default class BladeRunnerItem extends Item {
  ...
  get isPhysical() {
    return YZE.physicalItems.includes(this.type);
  }
  ...
  async roll() {
    ...
    if (this.system.consumable) {
      ...
      ui.notifications.info(
        game.i18n.format('YZE.ItemConsumeNotif', {
          name: this.name,
          qty: consumed,
        }),
      );
    ...
    if (this.actions.length > 1) {
      actionId = await BladeRunnerDialog.choose(
        ...
        `${this.detailedName}: ${game.i18n.localize('YZE.DIALOG.ChooseAction')}`,
      );
    }
    else {
    ...
    if (this.isOffensive) {
      ...
      if (this.attacks.length > 1) {
        attackId = await BladeRunnerDialog.choose(
          ...
          `${this.detailedName}: ${game.i18n.localize('YZE.DIALOG.ChooseAttack')}`,
        );
      }
      else {
  ...
  async _rollSpecial(value) {
    ...
    const execute = autoArmorRoll || await Dialog.confirm({
      title,
      content: `<p>${game.i18n.format('YZE.ROLLER.RollSpecialHint', {
        name: `<b>${this.name}</b>`,
      })}</p>`,
    });
    if (!execute) return;
