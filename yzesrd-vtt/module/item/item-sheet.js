...
import { ITEM_TYPES, SETTINGS_KEYS, SYSTEM_ID } from '../system/constants.js';
import { enrichTextFields } from '../utils/string-util.js';

/**
 * Year Zero Engine SRD RPG Item Sheet.
...
export default class BladeRunnerItemSheet extends foundry.appv1.sheets.ItemSheet {
  ...
  _getHeaderButtons() {
    ...
    if (this.item.hasAction && this.item.actor) {
      myButtons.push({
        label: game.i18n.localize('YZE.SHEET_HEADER.ItemRoll'),
        class: 'item-roll',
        icon: 'fas fa-dice',
    ...
    if (this.item.type === ITEM_TYPES.ARMOR) {
      myButtons.push({
        label: game.i18n.localize('YZE.SHEET_HEADER.ArmorRoll'),
        class: 'item-roll',
      ...
      myButtons.push({
        label: game.i18n.localize('YZE.SHEET_HEADER.BlastPowerRoll'),
        class: 'blast-roll',
        icon: 'fas fa-bomb',
    ...
    myButtons.push({
      label: game.i18n.localize('YZE.SHEET_HEADER.ItemPost'),
      class: 'item-post',
  ...
  _onAddItemAction(event) {
    ...
    const itemAction = new ItemAction(null, {
      name: game.i18n.localize('YZE.ItemNewAction'),
      // item: this,
    });
  ...
  _onAddItemAttack(event) {
    ...
    const itemAttack = new ItemAttack({
      name: game.i18n.localize('YZE.ItemNewAttack'),
      // item: this,
    });
  ...
  _onAddActiveEffect(event) {
    ...
    return this.item.createEmbeddedDocuments('ActiveEffect', [{
      name: game.i18n.localize('YZE.ACTIVE_EFFECT.New'),
  ...
  _onBlastChange(event) {
    ...
    if (!(blast in YZE.blastPowerMap)) return;
    if (this.item.attacks.length <= 0) return;
    const { damage, crit } = YZE.blastPowerMap[blast];
