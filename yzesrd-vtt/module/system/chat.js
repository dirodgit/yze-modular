import { ACTOR_TYPES, DAMAGE_TYPES, SYSTEM_ID } from './constants.js';
...
export function addChatMessageContextOptions(_html, options) {
  ...
  options.push({
    name: game.i18n.localize('YZE.CHAT_ACTION.ApplyDamage'),
    icon: YZE.Icons.buttons.attack,
  ...
  for (const defenderToken of defenderTokens) {
    if (defenderToken.actor.type === ACTOR_TYPES.LOOT) {
      ui.notifications.info(
        game.i18n.format('YZE.COMBAT.SkippedDefender', {
          name: `<b>${defenderToken.name}</b>`,
        }) + ' <i>(' + game.i18n.localize('YZE.COMBAT.CannotBeDamaged') + ')</i>',
      );
      continue;
    }
    if (!s) break;
    ...
    if(defenderTokens.size > 1 && !boom) {
      n = await BladeRunnerDialog.rangePicker({
        title: game.i18n.localize('YZE.DIALOG.AssignSuccesses'),
        description: game.i18n.format('YZE.DIALOG.AssignSuccessesHint', {
          name: `<b>${defenderToken.name}</b>`,
        }),
        value: s,
      });
...
export function addChatListeners(html) {
  html.addEventListener('click', ev => {
    if (ev.target.closest('.yze-modular-display-manual')) game.yzmodular.macros.displayManual.call(ev.target, ev);
  });
  html.addEventListener('click', ev => {
...
async function _onCritApply(event) {
  ...
  if (!item || !actor) {
    ui.notifications.error(game.i18n.localize('YZE.COMBAT.CritApplyErrorMessage'));
    return;
  }
  const itemDoc = await foundry.utils.fromUuid(item);
  const actorDoc = await foundry.utils.fromUuid(actor);
  if (!itemDoc || !actorDoc) {
    ui.notifications.error(game.i18n.localize('YZE.COMBAT.CritApplyErrorNotFound'));
    return;
  }
