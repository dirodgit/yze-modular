...
>${YZE.Icons.links.rolltable}${title}</a>`;
...
>${YZE.Icons.links.choice}${result}</a>`;
...
async function actorEnricher(match, _options) {
  ...
  // Builds attributes.
  const attributes = [];
  for (const attr of YZE.attributes) {
    const value = sys.attributes[attr].value;
    const text = game.i18n.localize(`YZE.ATTRIBUTE.${attr.toUpperCase()}`)
      + ` ${YZE.dieMap.get(+value)}`;
    attributes.push(text);
  }

  // Builds skills.
  const skills = [];
  for (const sk of YZE.skills) {
    const value = sys.skills[sk].value;
    if (value <= 6) continue;
    const text = game.i18n.localize(`YZE.SKILL.${sk.capitalize()}`)
      + ` ${YZE.dieMap.get(+value)}`;
    skills.push(text);
  }
...
<div class="flbr-table green" style="height: initial; max-width: 40%;">
  <table>
    <tbody>
      ...
      <tr><td>TYPE: <b>${game.i18n.localize(YZE.damageTypes[atk.damageType])}</b></td></tr>
      <tr><td>MIN. RANGE: <b>${game.i18n.localize(YZE.ranges[atk.range?.min])}</b></td></tr>
      <tr><td>MAX. RANGE: <b>${game.i18n.localize(YZE.ranges[atk.range?.max])}</b></td></tr>
      <tr><td>AVAILABILITY: <b>${game.i18n.localize(YZE.availabilities[sys.availability])}</b></td></tr>
...
async function bladeRunnerSymbolEnricher(match) {
  ...
  symbolDoc.className = 'yze-modular-symbols';
