...
export function registerCustomItemTypesTemplates() {
  const customItemTypesTemplates = Object.values(YZE.customItemTypesTemplates);
...
function registerHandlebarsHelpers() {
  ...
  Handlebars.registerHelper('scoreSelector', function (target, options) {
    ...
    for (const [score, size] of YZE.scoreMap) {
  ...
  Handlebars.registerHelper('boxes', function (field, options) {
    ...
    for (let i = 0; i < max; i++) {
      if (i === 10) str += '<br/>';
      if (value > i) str += `${YZE.Icons.boxes.full}`;
      else str += `${YZE.Icons.boxes.empty}`;
    }
    if (loss < 0) {
      str += '<span class="loss">';
      for (; loss < 0; loss++) {
        str += `${YZE.Icons.boxes.lost}`;
      }
      str += '</span>';
    }
    str += '</a>';
  ...
  Handlebars.registerHelper('createNewItemButton', function (type) {
    const title = game.i18n.format('YZE.CreateNewItem', {
      type: game.i18n.localize(`TYPES.Item.${type}`),
    });
    const str = `<a class="btn item-create" data-type="${type}" data-tooltip="${title}">${YZE.Icons.buttons.plus}</a>`;
    return new Handlebars.SafeString(str);
  });
}
