...
export async function setupMacroFolder() {
  ...
  const folderName = YZE.systemMacroFolder;
...
async function _createBladeRunnerStatMacro(data, slot) {
  const folder = game.folders.find(f => f.type === 'Macro' && f.name === YZE.systemMacroFolder);
  const command = `game.yzmodular.macros.rollStat("${data.attribute}"`
    + (data.skill ? `, "${data.skill}"` : '')
    + ');';
  const actor = await foundry.utils.fromUuid(data.uuid);
  if (!actor) return;

  const commandName = game.i18n.format('YZE.MACRO.RollStat', {
    stat: game.i18n.localize(data.skill
      ? `YZE.SKILL.${data.skill.capitalize()}`
      : `YZE.ATTRIBUTE.${data.attribute.toUpperCase()}`,
    ),
  });
...
async function _createBladeRunnerActionMacro(data, slot) {
  const folder = game.folders.find(f => f.type === 'Macro' && f.name === YZE.systemMacroFolder);
  const command = `game.yzmodular.macros.rollAction("${data.action}");`;
  const actor = await foundry.utils.fromUuid(data.uuid);
  if (!actor) return;

  const action = YZE.Actions.find(a => a.id === data.action);
  const commandName = game.i18n.format('YZE.MACRO.RollAction', {
...
async function _createBladeRunnerItemMacro(item, slot) {
  const folder = game.folders.find(f => f.type === 'Macro' && f.name === YZE.systemMacroFolder);
  const command = `game.yzmodular.macros.rollItem("${item.name}");`;
...
export async function rollAction(actionKey) {
  if (!game.yzmodular.actions.has(actionKey)) {
    return ui.notifications.error(`Action "${actionKey}" not found!`);
  }
  return game.yzmodular.actions.get(actionKey).execute(await getActiveActor());
}
...
export async function rollItem(itemName) {
  ...
  if (items.length > 1) {
    ui.notifications.warn(game.i18n.format('YZE.MACRO.MultipleItems', {
      actor: actor.name,
      item: itemName,
    }));
  }
  else if (items.length === 0) {
    return ui.notifications.warn(game.i18n.format('YZE.MACRO.NoItem', {
      actor: actor.name,
      item: itemName,
    }));
  }
...
export function showRollDialog(data = {}) {
  return game.yzmodular.roller.create({
    title: 'Generic Roll',
