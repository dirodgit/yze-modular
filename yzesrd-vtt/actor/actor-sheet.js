 ...
 */
export default class YzeSRDActorSheet extends foundry.appv1.sheets.ActorSheet {
  ...
  async _onDropItemCreate(itemData) {
    ...
    const alwaysAllowedItems = YZSRD.physicalItems;
    const allowedItems = YZSRD.allowedItems;
    let allowed = true;
    ...
    if (!allowed) {
      const msg = game.i18n.format('YZSRD.ActorSheet.NotifWrongItemType', {
  ...
  _getHeaderButtons() {
    ...
    const myButtons = [
      {
        label: game.i18n.localize('YZSRD.SHEET_HEADER.GenericRoll'),
        class: 'custom-roll',
        icon: 'fas fa-dice',
  ...
  _onItemCreate(event) {
    ...
    const itemName = game.i18n.format('YZSRD.NewItem', {
      type: game.i18n.localize(`TYPES.Item.${type}`),
    });
  ...
  _onAddActiveEffect(event) {
    ...
    return this.actor.createEmbeddedDocuments('ActiveEffect', [{
      label: game.i18n.localize('YZSRD.ACTIVE_EFFECT.New'),
