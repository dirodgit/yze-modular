 ...
 * @extends {Actor}
 */
export default class YzeSRDActor extends Actor {
    ...
    switch (this.type) {
      case ACTOR_TYPES.CHAR:
        return [ACTOR_SUBTYPES.PC, ACTOR_SUBTYPES.REPLICANT_REBELLION].includes(this.system.subtype)
          ? YZSRD.maxPushMap[this.system.nature]
          : 0;
      default:
        return 1;
    }
  ...
  _prepareCapacities() {
    ...
    for (const cap of Object.values(CAPACITIES)) {
      /** @type {ActorCapacity} */
      const capacity = this.system[cap];
      const capData = YZSRD.capacitiesMap[cap];
      // Proceeds if it exists in the character.
      if (capacity && capData) {
        // Gets the nature modifier.
        const natureModifier = YZSRD.natureModifierMap[this.system.nature]?.[cap] ?? 0;
    ...
    switch (this.type) {
      case ACTOR_TYPES.CHAR:
        ...
        if (!this.system.attributes || !this.system.skills) {
          throw new TypeError(`YZSRD | "${this.type}" has No attribute nor skill`);
        }
        if (foundry.utils.isEmpty(this.system.skills)) {
          ...
          for (const attribute in this.system.attributes) {
            updateData[`system.attributes.${attribute}.value`] = YZSRD.startingAttributeLevel;
          }
          // Builds the list of skills and sets their default values.
          for (const skill in YZSRD.skillMap) {
            updateData[`system.skills.${skill}.value`] = YZSRD.startingSkillLevel;
          }
        }
        break;
      case ACTOR_TYPES.VEHICLE:
   ...
   * @param {YzeSRDActor} actor The actor to add to the crew
  ...
  async addVehicleOccupant(actor) {
    ...
    if (this.crew.has(actor.id)) {
      return ui.notifications.info('YZSRD.VEHICLE.NotifPassengerAlreadyPresent', { localize: true });
    }
  ...
  async updateCharacterManeuverability(value) {
    ...
    const mvr = YZSRD.vehicleAttribute;
  ...
  async roll(rollData = {}, options = {}) {
    ...
    if (!rollData.title) rollData.title = `${this.name}: ${game.i18n.localize('YZSRD.SHEET_HEADER.GenericRoll')}`;
  ...
  async rollStat(attributeKey, skillKey, options = {}) {
    ...
    const attributeName = game.i18n.localize(`YZSRD.ATTRIBUTE.${attributeKey.toUpperCase()}`);
    const skillName = skillKey ? game.i18n.localize(`YZSRD.SKILL.${skillKey.capitalize()}`) : null;
   ...
   */
  async rollResolve() {
    const title = game.i18n.localize('YZSRD.ROLLER.ResolveTest');
    const execute = await Dialog.confirm({
      title,
      content: `<p>${game.i18n.localize('YZSRD.ROLLER.ResolveTestHint')}</p>`,
    });
    if (!execute) return;
    ...
    if (roll.baneCount) {
      ui.notifications.info(game.i18n.format('YZSRD.ROLLER.ResolveTestFailed', {
        name: this.name,
      }), {
        permanent: true,
      });
  ...
  async rollRamming() {
    ...
    const driving = YZSRD.vehicleSkill;
    const hullDamage = Math.ceil(this.system.hull.max / 2);

    return this.roll({
      title: `${this.name} | ${actor.name}: `
        + `${game.i18n.localize('YZSRD.VEHICLE.Action.Ramming')} `
        + `(${game.i18n.localize(`YZSRD.SKILL.${driving.capitalize()}`)})`,
      actor: actor,
      attributeKey: YZSRD.vehicleAttribute,
      skillKey: driving,
  ...
  async _applyDamage(damage, capacity, ignoreArmor = false) {
    ...
    if (!(capacity in this.system)) {
      throw new Error(`YZSRD | YzeSRDActor.applyDamage → Non-existent capacity "${capacity}"`);
    }
  ...
  async getArmorAblation() {
    ...
    // For vehicles:
      ...
      const armorRoll = game.bladerunner.roll.create(`2d${this.system.armor}p0`, {}, {
        name: `${this.name}: ${game.i18n.localize('YZSRD.ItemArmor')}`,
        yzur: true,
      });
  ...
  async crashVehicle(massive = false) {
    ...
    let formula = massive ? YZSRD.vehicleMassiveCrashDamage : YZSRD.vehicleCrashDamage;

    const toCrash = await Dialog.confirm({
      title: `${this.name}: ${game.i18n.localize('YZSRD.VEHICLE.Action.Crash')}`,
      content: game.i18n.format('YZSRD.VEHICLE.Action.CrashDialog', {
    ...
    const crashDamageRoll = foundry.dice.Roll.create(formula, rollData, {
      name: `${this.name}: ${game.i18n.localize('YZSRD.VEHICLE.Action.Crash')}`,
    });
    await crashDamageRoll.roll();
    const crashDamageRollMessage = await crashDamageRoll.toMessage({
      flavor: `${game.i18n.localize('YZSRD.VEHICLE.Action.Crash')}: ${formula}`,
    });
  ...
  async explodeVehicle() {
    ...
    const toExplode = await Dialog.confirm({
      title: `${this.name}: ${game.i18n.localize('YZSRD.VEHICLE.Action.Explode')}`,
      content: game.i18n.localize('YZSRD.VEHICLE.Action.ExplodeHint'),
    });
    if (!toExplode) return;

    const blastPower = YZSRD.vehicleExplosionBlastPower;
    const blast = YZSRD.blastPowerMap[YZSRD.vehicleExplosionBlastPower];
    ...
    const blastRoll = game.bladerunner.roll.create(`2d${blastPower}p0`, {}, {
      damage: blast.damage,
      damageType,
      damageTypeName: game.i18n.localize(YZSRD.damageTypes[damageType]),
      crit: blast.crit,
      yzur: true,
    });
    await blastRoll.roll();
    const blastRollMessage = await blastRoll.toMessage({
      flavor: game.i18n.localize('YZSRD.VEHICLE.Action.Explode'),
    });
  ...
  async _drawCritForCharacter(damageType, severity, critRollFormula) {
    ...
    const { results } = await YzeSRDDialog.drawTable(tables, {
      title: `${this.name}: ${game.i18n.localize('YZSRD.CRIT.DrawCrit')}`,
    ...
    const itemUuid = await YzeSRDDialog.choose(
      results
        .sort((a, b) => b.range[0] - a.range[0])
        .map(r => [r.documentUuid, `${r.name} (${r.range[0]})`]),
      `${this.name}: ${game.i18n.localize('YZSRD.CRIT.ChooseCrit')}`,
    ...
    const resultText = await foundry.applications.ux.TextEditor.enrichHTML(
      game.i18n.format('YZSRD.CHAT.CritInflicted', {
        name: `<b>${this.name}</b>`,
        crit: `@UUID[${crit.uuid}]`,
      }));
    const content = await foundry.applications.handlebars.renderTemplate(template, {
  ...
  async _drawCritForVehicle(severity, critRollFormula) {
    ...
    const { results } = await YzeSRDDialog.drawTable(tables, {
      title: `${this.name}: ${game.i18n.localize('YZSRD.CRIT.DrawVehicleDamage')}`,
    ...
    const resultIndex = await YzeSRDDialog.choose(
      results
        .sort((a, b) => b.range[0] - a.range[0])
        .map((r, i) => {
      ...
      `${this.name}: ${game.i18n.localize('YZSRD.CRIT.ChooseVehicleDamage')}`,
