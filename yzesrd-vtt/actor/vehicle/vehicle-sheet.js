import YzeSRDActorSheet from '../../actor/actor-sheet';
 ...
 * @extends {YzeSRDActorSheet} Extends the BR ActorSheet
 */
export default class YzeSRDVehicleSheet extends YzeSRDActorSheet {
  ...
  async _onDropActor(event, data) {
    ...
    return ui.notifications.info(`YZSRD.VEHICLE.NotifDropCrew${actor.type.capitalize()}`, { localize: true });
  }
   ...
   * @param {YzeSRDActor} actor
   */
  async dropCrew(actor) {
    if (this.vehicle.crew.full) {
      return ui.notifications.info('YZSRD.VEHICLE.NotifCrewFull', { localize: true });
    }
    if (
      ...
      const toCopy = await Dialog.confirm({
        title: game.i18n.localize('YZSRD.VEHICLE.CreateActor'),
        content: game.i18n.format('YZSRD.VEHICLE.CreateActorHint', {
          name: actor.name,
        }),
      });
      if (toCopy) {
  ...
  activateListeners(html) {
    ...
    new foundry.applications.ux.ContextMenu.implementation(html[0], '.hull', [
      {
        name: game.i18n.localize('YZSRD.VEHICLE.IncreaseHull'),
        icon: YZSRD.Icons.buttons.plus,
        callback: () => this.vehicle.update({
          'system.hull.max': this.vehicle.system.hull.max + 1,
        }),
        condition: () => this.vehicle.system.hull.max < YZSRD.maxVehicleHull,
      },
      {
        name: game.i18n.localize('YZSRD.VEHICLE.DecreaseHull'),
        icon: YZSRD.Icons.buttons.minus,
        callback: () => this.vehicle.update({
          'system.hull.max': this.vehicle.system.hull.max - 1,
        }),
    ...
    new foundry.applications.ux.ContextMenu.implementation(html[0], '.vehicle-seat:not(.button)', [
      {
        name: game.i18n.localize('YZSRD.VEHICLE.EditPassenger'),
        icon: YZSRD.Icons.buttons.edit,
        callback: el => game.actors.get(el.data('occupant-id'))?.sheet.render(true),
        condition: el => el.hasClass('occupied'),
      },
      {
        name: game.i18n.localize('YZSRD.VEHICLE.RemovePassenger'),
        icon: YZSRD.Icons.buttons.delete,
        callback: el => this.vehicle.removeVehicleOccupant(el.data('occupant-id')),
        condition: el => el.hasClass('occupied'),
      },
      {
        name: game.i18n.localize('YZSRD.VEHICLE.DeleteSeat'),
        icon: YZSRD.Icons.buttons.remove,
        callback: () => this.vehicle.update({
  ...
  _onManeuverabilityRoll(event) {
    ...
    return this.vehicle.roll({
      title: `${this.vehicle.name}: ${game.i18n.localize('YZSRD.ATTRIBUTE.MVR')}`,
  ...
  _onVehicleArmorRoll(event) {
    event.preventDefault();
    return this.vehicle.roll({
      title: `${this.vehicle.name}: ${game.i18n.localize('YZSRD.ItemArmor')}`,
